...
- **Código promocional obligatorio al registrarse:** campo `codigoPromocional` en `RegisterForm.tsx`, validado dentro de `registrarUsuario()` contra dos códigos privados en `src/modules/auth/constants/promociones.ts` — **ninguno de los dos valores reales va en placeholders de inputs, textos de ayuda, ni en esta documentación**; el placeholder del input dice genéricamente "Código de tu curso":
  ...
  - `CODIGO_PROMOCIONAL_ADMIN`: no requiere que el correo esté en ninguna lista — es la única forma hoy de crear una cuenta con `rol: 'admin'` (acceso al panel de administración) sin editar la base de datos a mano. Pensado para Carlos, Alicia y facilitadores, no para estudiantes.

  Al validarse cualquiera de los dos códigos, se guarda `finPeriodoGratuito` (`obtenerFinPeriodoGratuito()`, fecha fija `INICIO_PERIODO_GRATUITO` + 1 mes — **igual para todos los usuarios**, no depende de la fecha de registro de cada quien) en el perfil. **Pendiente:** no hay todavía ningún bloqueo real de acceso cuando el período vence (eso depende de la integración de TiloPay, ver sección Pagos) — por ahora Perfil solo muestra cuántos días quedan o que ya venció, sin restringir el uso de la app.
