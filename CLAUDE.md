...
- **Código promocional obligatorio al registrarse:** campo `codigoPromocional` en `RegisterForm.tsx`, validado dentro de `registrarUsuario()` contra dos códigos privados en `src/modules/auth/constants/promociones.ts` — **ninguno de los dos valores reales va en placeholders de inputs, textos de ayuda, ni en esta documentación**; el placeholder del input dice genéricamente "Código de tu curso":
  - `CODIGO_PROMOCIONAL_VALIDO` (código de estudiante): además de coincidir el código, el correo ingresado debe estar en la lista de estudiantes inscritos en el curso — se valida contra `CORREOS_ESTUDIANTES_HASH` (`constants/estudiantesInscritos.ts`), que guarda el **hash SHA-256** de cada correo normalizado (minúsculas + trim), no el correo en texto plano — decisión explícita para no exponer datos personales reales de ~50 estudiantes en el código fuente que vive en GitHub. El hasheo corre en el navegador vía `sha256Hex()` (`src/shared/lib/hash.ts`, Web Crypto nativo). Si el código no coincide con ninguno de los dos, o el correo no está en la lista, `AuthError` con el mensaje correspondiente.
  - `CODIGO_PROMOCIONAL_ADMIN`: no requiere que el correo esté en ninguna lista — es la única forma hoy de crear una cuenta con `rol: 'admin'` (acceso al panel de administración) sin editar la base de datos a mano. Pensado para Carlos, Alicia y facilitadores, no para estudiantes. **Es secreto** — solo lo conocen los admins, nunca se documenta el valor real ni se comparte con estudiantes.

  Solo las cuentas de estudiante (`rol: 'student'`) reciben `finPeriodoGratuito` (`obtenerFinPeriodoGratuito()`, fecha fija `INICIO_PERIODO_GRATUITO` + 1 mes — **igual para todos los estudiantes**, no depende de la fecha de registro de cada quien). Las cuentas admin **no** tienen este campo (queda `undefined`) — significa acceso completo, ilimitado y sin costo desde el registro, sin período de prueba ni suscripción; `ProfileScreen.tsx` y `SuscripcionScreen.tsx` muestran una tarjeta "Acceso completo" en vez del contador de días/botón de suscripción cuando este campo no existe. Aun así, **un admin es un usuario tradicional**: usa Biblia, Finanzas y Perfil exactamente igual que un estudiante, solo que además ve el tab Admin y nunca ve paywall. **Pendiente:** no hay todavía ningún bloqueo real de acceso cuando el período de un estudiante vence (eso depende de la integración de TiloPay, ver sección Pagos) — por ahora Perfil solo muestra cuántos días quedan o que ya venció, sin restringir el uso de la app.
...
- **Decisión pendiente de implementar (2026-07-27):** `tech@talentaapp.com` será una cuenta **SUPER admin**, distinta de los admins regulares (Carlos, Alicia, facilitadores) — probablemente con permisos que un admin normal no tiene (aún sin definir cuáles, ya que el panel de administración hoy es solo un placeholder sin funcionalidad real). Por ahora `UserRole` sigue siendo solo `'student' | 'admin'` — **no** agregar un rol `'superadmin'` hasta construir el panel de admin real y saber qué debe diferenciar en la práctica; hacerlo antes sería código sin ningún uso.
