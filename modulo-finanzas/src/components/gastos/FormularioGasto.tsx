...
function camposDesde(g: Gasto): Campos {
...
export default function FormularioGasto({ gastoInicial, onGuardado, onCancelar }: Props) {
  ...
  const [form, setForm] = useState<Campos>(() =>
    gastoInicial ? camposDesde(gastoInicial) : { ...INICIAL, usuarioId: '' }
  )
    ...
    try {
      if (gastoInicial) {
        await gastosRepository.actualizar(gastoInicial.id, {
          ...
          tipoPago:         form.tarjetaId ? 'tarjeta' : 'contado',
          fecha:            form.fecha,
        ...
        const gasto: Gasto = {
          ...
          monto:                Number(form.monto),
          moneda:               form.moneda,
          tipoCambioAlMomento:  tipoCambio,
          categoriaId:          form.categoriaId,
          tarjetaId:            form.tarjetaId || undefined,
          usuarioId:            form.usuarioId,
          tipoPago:             form.tarjetaId ? 'tarjeta' : 'contado',
  ...
  return (
    ...
    >
          ...
          {CATEGORIAS.map((cat) => (
            <button
              ...
              className={cn(
                ...
                form.categoriaId === cat.id ? 'border-transparent scale-105' : 'border-border bg-secondary'
              )}
              style={form.categoriaId === cat.id
      ...
      {/* Usuario */}
      <Campo label="¿Quién pagó?" error={errores.usuarioId}>
                    ...
                    >
                      <div className="flex flex-col gap-2 pt-1">
                        <div className="flex justify-between text-xs text-muted-foreground px-1">
                          <span>{usuarioPagador?.nombre ?? 'Quien pagó'}</span>
